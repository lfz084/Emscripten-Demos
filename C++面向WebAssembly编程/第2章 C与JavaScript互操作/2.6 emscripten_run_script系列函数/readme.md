...
上述代码中emscripten_run_script("console.log(42);");执行效果等价于在JavaScript中执行console.log(42);。
...
上述代码先定义了一个方法my_print()，然后调用它输出了“Hello!”

tips 为了避免使用转义符方便阅读，上述例子定义字符串时，使用了C++11标准的raw字符串定义方法（R前缀），在使用emcc命令编译时，必须增加-std=c++11参数，完整的命令行为：
...
The answer is 42.
我们在胶水代码中可以看到辅助函数emscripten_run_script_string()实现代码如下：
...
该函数在C/C++内存中分配了空间，用于保存传入脚本执行后返回的字符串。并且从代码中不难发现，多次调用emscripten_run_script_string()时，后面调用的结果有可能覆盖前面调用的结果——因为me.buffer是重复使用的。
