...
html中声明了id为myCanvas的Canvas元素，在每帧更新时，从Emscripten运行时Module中取出图像数据，创建ImageData对象imgData，并将imgData通过CanvasRenderingContext2D对象ctx更新到Canvas上去。整体的数据流程如下图所示：
...
事实上Canvas对象已经内置包括圆、矩形、扇形等多种几何图形的绘制方法，上述例程使用自定义的draw_circle()函数更多是为了演示在如何在C语言中操作位图图像，以及如何提取图像并更新到Canvas。Canvas、ImageData、CanvasRenderingContext2D等相关对象的详细资料，可参考MDN。